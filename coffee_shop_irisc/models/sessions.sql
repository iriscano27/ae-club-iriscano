...
select * 
  , min(timestamp) over (partition by visitor_id,session_number) as session_start_time 
  , max(timestamp) over (partition by visitor_id,session_number) as session_end_time 
from add_session